...
BEST_GAME=$($PSQL "SELECT MIN(num_of_guess) FROM users INNER JOIN games USING(user_id) WHERE username='$USERNAME'")
...
INSERT_GAME=$($PSQL "INSERT INTO games(num_of_guess, user_id) VALUES ($GUESS, $USER_ID)")